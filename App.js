...
import { Alert, BackHandler, Linking, PermissionsAndroid, Platform, StatusBar } from 'react-native';
import {
  NavigationContainer,
  DarkTheme,
  DefaultTheme,
...
import { SET_ALERT, SET_FETCHED, SET_NOINTERNET_MODAL, SET_SPLASH, SET_SPLASH_URL, SET_YEAR_STRING } from './app/store/actions/types';
...
import { initConnection, clearProductsIOS, clearTransactionIOS } from 'react-native-iap';
...
const App = () => {
  ...
  const { mobileTheme: theme, minAppVerison } = useSelector((state) => state.brandingReducer.brandingData);
  const { orgId } = useSelector(state => state.activeOrgReducer);
  const { key } = useSelector(state => state.publishableKey);
  const { token, isAuthenticated, user, userId } = useSelector(state => state.authReducer)
  ...
  const appInformation = async (appInfoType) => {
    ...
    try {
      ...
      if (res.data.data.splashScreenContentUrl == null) {
        ...
        await dispatch({ type: SET_SPLASH_URL, payload: res.data.data.splashScreenContentUrl })
  ...
  const makeDownloadsFolder = () => {
    ...
    RNFetchBlob.fs
      .mkdir(encrptedFolder)
      .catch(err => {
        // console.log(err);
      });
    RNFetchBlob.fs
      .mkdir(decrptedFolder)
      .catch(err => {
        // console.log(err);
      });
